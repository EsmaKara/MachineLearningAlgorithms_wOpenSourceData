...
from sklearn.decomposition import PCA  # PCA'yı içeri aktardık
import time
import joblib
import warnings
...
print(df['property_type_branded_ratio'].to_list())
...
# PCA'yı ekleyelim
pca = PCA(n_components=0.80)  # Toplam varyansın %95'ini açıklayacak kadar bileşen seçiyoruz
X_train_pca = pca.fit_transform(X_train_num_scaled)
X_test_pca = pca.transform(X_test_num_scaled)

print(f"PCA sonrası eğitim veri boyutu: {X_train_pca.shape}")
print(f"PCA sonrası test veri boyutu: {X_test_pca.shape}")
